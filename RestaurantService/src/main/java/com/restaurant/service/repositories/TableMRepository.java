...
//	            + " WHERE r.tableM.id = t.id)")
	@Query("SELECT t FROM TableM t "+
		       ...
		       "AND NOT EXISTS (SELECT 1 FROM Reservation r WHERE r.date = :date AND r.time = :time AND r.tableM.id = t.id)")
		public List<TableM> listTableByPerson(@Param("person_number") String person_number, @Param("date") String date,@Param("time") String time);
}
